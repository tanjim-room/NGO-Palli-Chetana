            ...
            <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden pc-card">
                    <div class="position-relative overflow-hidden" style="height: 460px;">
                        ...
                        @else
                        <div class="w-100 h-100 d-flex align-items-center justify-content-center" style="background: var(--pc-gray-light);">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($member->name) }}&background=2a8547&color=fff&size=200&font-size=0.4"
                                 class="rounded-circle" style="width: 150px; height: 150px; object-fit: cover;" alt="{{ $member->name }}">
                        </div>
                        @endif
                    </div>
                    <div class="card-body p-4 text-center">
                        <h5 class="fw-bold mb-1">{{ $member->name }}</h5>
                        <p class="mb-1" style="color: var(--pc-primary); font-weight: 500; font-size: 0.9rem;">{{ $member->designation }}</p>
                        @if($member->profession)
                        <p class="text-muted small mb-1"><i class="bi bi-briefcase me-1"></i>{{ $member->profession }}</p>
                        @endif
                        @if($member->gender)
                        <p class="text-muted small mb-1"><i class="bi bi-person me-1"></i>{{ $member->gender }}</p>
                        @endif
                        @if($member->duration_of_involvement)
                        <p class="text-muted small mb-2"><i class="bi bi-calendar-range me-1"></i>{{ $member->duration_of_involvement }}</p>
                        @endif
                        @if($member->bio)
